...
@app.command(help="Perform a full scan of the target sequence.")
def fullscan(
    ...
    topmodel_path = os.path.join(project_dir, "topmodel.pkl")
    output_path = os.path.join(project_dir, "fullscan_"+batch_name+".csv")

    full_scan(esm_model, topmodel_path, tokenizer, wt_sequence, path, output_path)
